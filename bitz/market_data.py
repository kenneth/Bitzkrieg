...
class L2Depth(MarketDataBase):
    """
    L2 depth. Only contains 5 level of aggregated depth.
    """
    def __init__(self):
        """
        Constructor
    ...
    def values(self):
        ...
        """
        return [self.date_time.strftime("%Y%m%d %H:%M:%S.%f"), 
                self.b1, self.b2, self.b3, self.b4, self.b5,
                self.a1, self.a2, self.a3, self.a4, self.a5,
                self.bq1, self.bq2, self.bq3, self.bq4, self.bq5,
                self.aq1, self.aq2, self.aq3, self.aq4, self.aq5]     
    

class LongDepth(MarketDataBase):
    """
    Long price depth. Container of date, time, bid and ask up to 5 levels
    """
...
class Snapshot(MarketDataBase):
    ...
    def __init__(self, exchange, instmt):
        """
        Constructor
        :param exch: Exchange name
        :param instmt: Instrument name
        ...
        self.instmt = instmt
        self.order_book = L2Depth()
        self.last_trade = Trade()
        self.update_type = Snapshot.UpdateType.NONE
